...
function showRiddle() {
  ...
  newRiddleText.textContent = question;
  
  const newRiddleAnswer = document.createElement("p"); // returns the first element within the document that matched the specificed selector 'p'
  newRiddleAnswer.textContent = answer;
...
function revealAnswers() {
  ...
  const answer = document.querySelector("#riddle-answer") // use # to use select id
  if (riddle) {
    if (answer.hidden) {
      answer.hidden = false;
    } else {
    ...
    alert("Please show me the riddle ~");
  }
}
